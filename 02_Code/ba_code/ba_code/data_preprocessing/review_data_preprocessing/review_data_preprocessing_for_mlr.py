...
for review_uri in ReviewUri:
    ...
    [author_levels, contributions, helpful_votes, author_distributions] = extract_important_properties_from_author_data(
        author_data)
    ...
    expression_0.append((1 / number_of_reviews) * np.sum(np.multiply(ratings, np.reciprocal(age_of_reviews_in_days))))
    ...
    expression_4.append((1 / number_of_reviews) * np.sum(reciprocal_of_contributions * ratings * author_distribution_levels[0]))
    expression_5.append((1 / number_of_reviews) * np.sum(reciprocal_of_contributions * ratings * author_distribution_levels[1]))
    expression_6.append((1 / number_of_reviews) * np.sum(reciprocal_of_contributions * ratings * author_distribution_levels[2]))
    expression_7.append((1 / number_of_reviews) * np.sum(reciprocal_of_contributions * ratings * author_distribution_levels[3]))
    expression_8.append((1 / number_of_reviews) * np.sum(reciprocal_of_contributions * ratings * author_distribution_levels[4]))
